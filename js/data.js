...
CYKL.bikeStates = Array.from({ length: CYKL.TOTAL_BIKES }, (_, i) => ({
  id: i + 1,
...
SEED_OCCUPIED.forEach(s => {
  const b = CYKL.bikeStates.find(b => b.id === s.id);
  if (b) {
    b.status = 'occupied';
    b.user = s.user;
    b.class = s.class;
...
/* ── COMPUTED STATS ─────────────────────────────────────────── */
CYKL.getStats = function () {
  const occupied = CYKL.bikeStates.filter(b => b.status === 'occupied').length;
  const blocked = CYKL.bikeStates.filter(b => b.status === 'blocked').length;
  const available = CYKL.bikeStates.filter(b => b.status === 'available').length;
  const totalOcc = occupied + blocked;
  const pct = Math.round((totalOcc / CYKL.TOTAL_BIKES) * 100);
